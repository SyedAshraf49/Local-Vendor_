...
const Login: React.FC = () => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (username.trim() && password.trim()) {
      ...
      const isVendor = userType === 'vendor' && username === 'vendor' && password === 'pass';

      if (isCustomer || isVendor) {
         login(username, userType);
      } else {
        alert('Invalid credentials. Use username "customer" or "vendor" and password "pass"');
      }
    }
  };
            ...
            <div className="flex">
                ...
                <div className="ml-3">
                    <p className="text-sm text-yellow-800 dark:text-yellow-200 leading-relaxed">
                        <span className="font-semibold block">Test Credentials</span>
                        <span>Customer: <code className="font-bold">customer / pass</code></span>
                        <br/>
                        <span>Vendor: <code className="font-bold">vendor / pass</code></span>
                    </p>
                </div>
            </div>
          </div>
